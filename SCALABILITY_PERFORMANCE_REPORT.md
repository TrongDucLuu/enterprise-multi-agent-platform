...
4. **Redis Multi-Tenant Candidate-Set Vector Cache (1.000 Entries Empirical Benchmark)**:
   ...
   - **Kết quả đo đạc thực tế (`scripts/benchmark_semantic_cache.py`)**:
     ...
     - Thời gian truy vấn tìm kiếm gần nhất (Vector Cosine Similarity): **p50 = 21.19ms**, **p95 = 21.55ms**, **p99 = 28.45ms** $\rightarrow$ **nhanh hơn 57x lần so với gọi trực tiếp LLM (1.200ms)**.
